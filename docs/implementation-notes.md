...
**対象バージョン: v0.5.0 (2026-08-18) / Phase 1〜4 すべて完了**
...
| `diary` | 食事日記(日送り・カレンダー・削除) | ✅ カレンダーは v0.4.1 |
| `weight` | 体重の記録とグラフ | ✅ |
| `exercise` | 運動の記録 | ✅ |
| `badges` | 連続記録とバッジ | ✅ |
...
- カレンダーを開いたまま日を移ったときは `renderDiary()` の中で選択位置を追従させる
...
`/tmp/test-slimquest.js` に jsdom + fake-indexeddb による自動テストがある(**282項目**、
